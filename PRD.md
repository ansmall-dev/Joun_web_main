...
| 항목 | 내용 |
|---|---|
| 문서 버전 | v1.3 (히어로 섹션에 공신력 배지 추가) |
| 작성일 | 2026-08-12 (최종 업데이트: 2026-08-20, v1.3) |
...
## 12. 개발 진행 현황 (2026-08-20 기준, v1.3)

### 12.0j v1.3 변경 사항 ✅ (2026-08-20)

**히어로 섹션에 공신력 배지(MARA·JP) 추가**

1. **배경** — 기존에는 MARA(Registered Migration Agent)·JP(Justice of the Peace) 공신력 배지가 푸터에만 노출됨. 방문자가 페이지 최상단(첫 화면)에서 바로 자격을 확인할 수 있도록 히어로 섹션에도 배지를 배치 요청
2. **구현** — `Hero.jsx`에 CTA 버튼(상담예약/서비스 살펴보기) 아래 골드 구분선 + MARA 로고·"Registered Migration Agent / MARN 0741800" 텍스트·JP 실 로고를 가로 배치. `content.js`에 `hero.badgeLabel` 영/한 키 추가(F-08 다국어 원칙 유지)
3. **레이아웃 반복 조정** — 최초 좌측 정렬 소형 버전 적용 후, 대표 피드백에 따라 **크기 확대**(MARA 로고 108px→150px, JP 로고 46px→66px, 라벨·값 폰트 확대) 및 **컨테이너 내 가운데 정렬**로 재조정. 모바일(~640px)은 줄바꿈 대응 축소 버전 유지
4. **검증** — 로컬 dev 서버에서 DOM 좌표 검사로 배지가 히어로 컨테이너 중앙에 정렬됨을 확인, 이미지 정상 로드 확인
...
| 2 | 히어로 | 풀스크린, 버건디 오버레이 + 골드 이탤릭 강조 헤드라인, 듀얼 CTA, SYDNEY·AUSTRALIA 라벨, CTA 하단 공신력 배지(MARA·JP, v1.3 신설) |
